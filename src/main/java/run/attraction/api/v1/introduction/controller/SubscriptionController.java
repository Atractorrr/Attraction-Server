...
public class SubscriptionController {
  ...
  public ApiResponse<Void> subscribeNewsletter(@PathVariable String userEmail, @PathVariable @NotNull @Min(1) Long newsletterId) {
  ...
  @PatchMapping("/{userEmail}/unsubscribe/{newsletterId}")
  @Operation(summary = "뉴스레터 구독취소", description = "특정 뉴스레터를 구독 취소할 수 있다.")
  public ApiResponse<Void> unsubscribeNewsletter(@PathVariable String userEmail, @PathVariable @NotNull @Min(1) Long newsletterId) {
    subscriptionService.unsubscribeNewsletter(userEmail, newsletterId);
  ...
  public ApiResponse<List<Category>> getUserSubscribedNewsletterCategories(@PathVariable String userEmail) {
    List<Category> userSubscribedNewsletterCategories = subscriptionService.getUserSubscribedNewsletterCategories(
        userEmail);
